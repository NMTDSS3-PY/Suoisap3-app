...
export default function QuanLyNhaMay() {
  ...
  function xuLyTaiFileHoSo(id, file) {
    if (!file) return;
    if (file.size > GIOI_HAN_FILE_HOSO) {
      alert(
        "Tệp quá lớn (giới hạn khoảng 700KB/tệp). Với tệp lớn hơn, vui lòng lưu trên Google Drive/Zalo Cloud và dán đường link vào tên hồ sơ hoặc ghi chú riêng."
      );
      return;
    }
    ...
    reader.onload = () => {
      setHoSo((prev) =>
        prev.map((x) =>
          x.id === id ? { ...x, file: { ten: file.name, loai: file.type, duLieu: reader.result } } : x
        )
      );
    };
  ...
  function xoaFileHoSo(id) {
    setHoSo((prev) => prev.map((x) => (x.id === id ? { ...x, file: undefined } : x)));
  }
              ...
              {banGiao
                ...
                <div key={b.id} className="rounded-lg border border-slate-200 bg-blue-50/50 p-4">
                  ...
                  <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-3">
                    ...
                    <label className="text-xs text-slate-500 flex flex-col gap-1">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setBanGiao((prev) =>
                            prev.map((x) => (x.id === b.id ? { ...x, nguoiNhan: e.target.value } : x))
                          )
                        }
                        placeholder="Họ tên"
                        className="bg-slate-100 rounded px-2 py-1.5 text-slate-900 text-sm border border-slate-300"
                  ...
                  <div className="grid sm:grid-cols-2 gap-4 mt-4 pt-4 border-t border-slate-200">
                    <ChuKyPad
                      label="Chữ ký người giao ca"
                      value={b.chuKyGiao}
                      onChange={(data) =>
                        setBanGiao((prev) => prev.map((x) => (x.id === b.id ? { ...x, chuKyGiao: data } : x)))
                      }
                    />
                    <ChuKyPad
                      label="Chữ ký người nhận ca"
                      value={b.chuKyNhan}
                      onChange={(data) =>
                        setBanGiao((prev) => prev.map((x) => (x.id === b.id ? { ...x, chuKyNhan: data } : x)))
                      }
                    />
                  </div>
                </div>
                  ...
                  <table className="w-full text-sm min-w-[480px]">
                    <thead>
                      <tr className="bg-blue-50 text-slate-500 text-left">
                        ...
                        <th className="p-3 font-medium">Tệp đính kèm</th>
                        <th className="p-3 font-medium w-12"></th>
                      </tr>
                    </thead>
                    <tbody>
                              ...
                              {h.file ? (
                                <div className="flex items-center gap-2">
                                  <a
                                    ...
                                    download={h.file.ten}
                                    title={h.file.ten}
                                    className="flex items-center gap-1 text-blue-600 hover:underline text-xs truncate max-w-[140px]"
                                  >
                                    <FileText size={13} className="shrink-0" /> {h.file.ten}
                                  </a>
                                  <button
                                    onClick={() => xoaFileHoSo(h.id)}
                                ...
                                <label className="flex items-center gap-1 text-xs text-blue-600 hover:underline cursor-pointer w-fit">
                                  <Upload size={13} /> Tải nội dung
                                  <input
                                    type="file"
                                    className="hidden"
                                    onChange={(e) => {
                                      xuLyTaiFileHoSo(h.id, e.target.files?.[0]);
                                      e.target.value = "";
                                    }}
                                  />
                                </label>
                      ...
                      {hoSo.filter((h) => (nhomHoSo === "thietbi" ? h.nhom === "thietbi" : h.phanLoai === nhomHoSo)).filter((h) => h.ten.toLowerCase().includes(timHoSo.toLowerCase())).length === 0 && (
                        <tr>
                          <td colSpan={4} className="p-4 text-center text-slate-500 text-sm italic">
